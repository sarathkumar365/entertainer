...
def create_app(token: str | None = None, live: bool | None = None) -> FastAPI:
    ...
    def add(body: AddRequest) -> dict[str, Any]:
        ...
        # is skipped in live mode. The verdict is still recorded and still
        # merges back, because it keys on the IMDb id.
        try:
            item_id, _ = add_title(engine, body.tmdb_id, body.kind, place=not use_live)
        except IngestError as exc:
            raise HTTPException(500 if "item space" in str(exc) else 404, str(exc)) from exc
